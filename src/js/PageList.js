...
const PageList = (argument = "") => {
  ...
  const preparePage = () => {
    ...
    const fetchList = (url, argument) => {
      ...
      fetch(`${finalURL}`)
        ...
        .then((response) => {
         ...
         fetch(`https://api.rawg.io/api/games/${article.id}?key=${API_KEY}`)
            .then((response) => response.json())
            .then((response) => {
    ...
    fetchList(`https://api.rawg.io/api/games?key=${API_KEY}&dates=${today},${oneYearofToday}&ordering=-added`);
...
btnSearch.addEventListener('click',function Value(){
  ...
  fetch(`https://api.rawg.io/api/games?key=${API_KEY}&search=${value}`)
  .then((response) => response.json())
  .then((response) => {
    response.results.forEach((article) => {
      let platformsName = [];
